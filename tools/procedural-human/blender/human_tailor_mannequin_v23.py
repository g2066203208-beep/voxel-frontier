...
SLICES=[
 (0.58,0.93,0.95),(0.63,0.94,0.96),(0.69,0.97,0.98),(0.75,0.99,0.99),
 (0.80,0.98,0.98),(0.835,0.92,0.94),(0.865,0.98,0.98),(0.91,1.02,1.01),
 (0.96,1.035,1.025),(1.00,1.025,1.02),
]
...
def build_cropped_surface():
 ...
 for old in used:
  ...
  x*=sx;y*=sy;V.append((x*TARGET_HEIGHT,y*TARGET_HEIGHT,q*TARGET_HEIGHT));Q.append((x,y,q))
 F=[]
 for f in rawF:
...
def staged_decimate(o,target):
 history=[]
 for i in range(8):
  cur=tri(o);history.append(cur)
  if cur<=int(target*1.04):break
  desired=target/cur
  # Blender's collapse modifier is much more reliable in several moderate passes
  # than in one extreme 1-2% collapse.
  ratio=max(0.15,min(0.72,desired))
  d=o.modifiers.new(f'LowPolyHuman_{i}','DECIMATE');d.decimate_type='COLLAPSE';d.ratio=ratio;d.use_collapse_triangulate=True
  bpy.ops.object.modifier_apply(modifier=d.name);o.data.validate(verbose=False,clean_customdata=True);o.data.update()
 final=tri(o);history.append(final);print('DECIMATE_HISTORY',history);return final
...
def rebuild(o,cut):
 ...
 o.data.update();before=tri(o);after=staged_decimate(o,TARGET_TRIS)
 for p in o.data.polygons:p.use_smooth=False
 o.data.update();return before,after
...
def clean_model():
 o,cut=mesh_obj();before,_=rebuild(o,cut);o=sanitize(o);t=topo(o);assert t['connected_components']==1 and t['boundary_edges']==0 and t['nonmanifold_edges']==0,t;return o,before,tri(o),t,cut
