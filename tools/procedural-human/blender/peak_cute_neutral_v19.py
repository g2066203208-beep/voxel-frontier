...
OUT=os.path.abspath(sys.argv[sys.argv.index('--')+1]) if '--' in sys.argv else os.path.abspath('artifacts/peak-cute-neutral')
os.makedirs(OUT,exist_ok=True)
HEIGHT=1.43;SIDES=14;VOXEL=.010;TARGET=960
TORSO=[(.565,.155,.112),(.620,.153,.112),(.700,.146,.109),(.795,.136,.103),(.860,.132,.101),(.930,.141,.106),(.995,.151,.110),(1.045,.156,.110),(1.080,.145,.103),(1.108,.112,.086)]
LEGS=[(.050,.062,.060),(.105,.055,.055),(.205,.061,.061),(.300,.067,.066),(.385,.060,.060),(.480,.071,.069),(.565,.080,.075)]
...
def scaffold():
 ...
 # Central pelvis blend eliminates the crotch pinch while remaining construction-only.
 ap(V,F,*ell((0,0,.585),(.142,.100,.095),18,7))
 for s in (-1,1):
  ap(V,F,*loft(LEGS,cx=s*.073));ap(V,F,*ell((s*.071,0,.565),(.086,.079,.086),14,5));ap(V,F,*ell((s*.073,-.034,.044),(.064,.084,.048),14,5))
...
def raw():
 V,F=scaffold();me=bpy.data.meshes.new('PeakCuteNeutralV19Scaffold');me.from_pydata(V,[],F);me.update();o=bpy.data.objects.new('PeakCuteNeutralV19',me);bpy.context.collection.objects.link(o);o.data.materials.append(mat('NeutralBody',(.79,.75,.73)));return o
...
def sanitize(o,name='PeakCuteNeutralV19Export'):
 # Rebuild from nothing but final positions + triangles. This intentionally drops
 # stale loop/custom-normal layers left by voxel remesh/decimate in Blender 4.0.
 o.data.calc_loop_triangles();verts=[tuple(v.co) for v in o.data.vertices];faces=[tuple(t.vertices) for t in o.data.loop_triangles]
 me=bpy.data.meshes.new(name+'Mesh');me.from_pydata(verts,[],faces);me.validate(verbose=True,clean_customdata=True);me.update(calc_edges=True)
 bm=bmesh.new();bm.from_mesh(me);bmesh.ops.recalc_face_normals(bm,faces=bm.faces);bm.to_mesh(me);bm.free();me.update()
 n=bpy.data.objects.new(name,me);bpy.context.collection.objects.link(n);n.data.materials.append(mat('NeutralBody',(.79,.75,.73)));[setattr(p,'use_smooth',True) for p in n.data.polygons]
...
def glb_check(path):
 ...
 assert doc and doc.get('meshes');prims=sum(len(m.get('primitives',[])) for m in doc['meshes']);assert prims>0
 return len(data),len(doc['meshes']),prims
...
def clean_model(): o,b,a,t=make();o=sanitize(o);t2=topo(o);assert t2['connected_components']==1 and t2['boundary_edges']==0 and t2['nonmanifold_edges']==0;return o,b,tr(o),t2
...
M={'style':'peak-like-cute-neutral-single-shell-v19','original_geometry':True,'external_character_mesh':False,'neutral_only':True,'male_female_split':False,'hair':False,'face_addons':False,'clothing':False,'props':False,'height_m':HEIGHT,'head_height_m':.356,'visual_head_ratio':round(HEIGHT/.356,3),'tris_before_decimate':b,'tris':a,'single_continuous_shell':T['connected_components']==1 and T['boundary_edges']==0 and T['nonmanifold_edges']==0,**T,'voxel_size_m':VOXEL,'target_tris':TARGET,'glb_bytes':gb,'glb_meshes':gm,'glb_primitives':gp,'glb_valid_mesh':gp>0,'topology_method':'soft semantic scaffold -> voxel union -> relax -> low-poly -> fresh sanitized triangle mesh -> GLB','blender_version':'.'.join(map(str,bpy.app.version))};json.dump(M,open(os.path.join(OUT,'metrics.json'),'w'),indent=2);json.dump({'torso':TORSO,'legs':LEGS,'arm_radii':ARMS,'head':{'center':[0,-.004,1.255],'radii':[.184,.171,.178]},'central_pelvis_blend':{'center':[0,0,.585],'radii':[.142,.100,.095]}},open(os.path.join(OUT,'section_spec.json'),'w'),indent=2);print('PEAK_CUTE_NEUTRAL_V19_METRICS',json.dumps(M))