...
import { loadAdultPhenotypeBasis } from './morph-basis.js';
...
const sliderDefs = [
  ...
  ['armLength','手臂长度','',-1,1,0.01],['bodyFat','软组织/体重','',-1,1,0.01],
...
const camera=new THREE.PerspectiveCamera(35,1,0.01,100); camera.position.set(2.15,1.12,2.85);
...
const floor=new THREE.Mesh(new THREE.CircleGeometry(1.0,64),new THREE.MeshStandardMaterial({color:0x222831,roughness:0.95})); floor.rotation.x=-Math.PI/2; floor.receiveShadow=true; scene.add(floor);
...
async function load(){try{$('#status').textContent='Loading CC0 master human mesh…';const res=await fetch(MASTER_URL,{cache:'force-cache'});if(!res.ok)throw new Error(`HTTP ${res.status}`);const parsed=parseOBJ(await res.text());state.master=canonicalize(parsed);let loaded=0;$('#status').textContent='Loading real phenotype displacement fields 0/24…';state.master.phenotypeBasis=await loadAdultPhenotypeBasis(state.master,()=>{$('#status').textContent=`Loading real phenotype displacement fields ${++loaded}/24…`;});buildUI();applyPreset('male');}catch(err){console.error(err);$('#status').textContent=`Load failed: ${err.message}`;$('#error').hidden=false;}} load();
