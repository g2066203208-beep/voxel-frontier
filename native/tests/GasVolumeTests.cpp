...
[[noreturn]] void fail(std::string_view message) {
    std::cerr << "GAS VOLUME TEST FAILURE: " << message << '\n';
    std::exit(1);
}
...
void testPressureRespondsToVolumeAndTemperature() {
    vf::GasVolumeState gas{};
    gas.volumeM3 = 1.0;
    gas.dryGasMassKg = 1.2;
    gas.temperatureK = 293.15;
...
void testCoolingCreatesFogAndWarmingEvaporatesIt() {
    vf::GasVolumeState gas{};
    gas.volumeM3 = 1.0;
    gas.dryGasMassKg = 1.1;
    gas.waterVaporMassKg = 0.020;
    gas.temperatureK = 278.15;
    ...
    const double condensed = gas.condensedFogMassKg;
    gas.temperatureK = 315.0;
...
void testLeakMovesPressureTowardAmbient() {
    vf::GasVolumeState gas{};
    gas.volumeM3 = 0.5;
    gas.dryGasMassKg = 1.5;
    gas.temperatureK = 293.15;

    vf::GasLeak leak{};
    leak.openingAreaM2 = 1.0e-4;
    leak.externalPressurePa = 101325.0;
    leak.externalDensityKgPerM3 = 1.225;
    ...
    const double finalDifference = std::abs(vf::gasPressurePa(gas) - leak.externalPressurePa);
    require(finalDifference < initialDifference, "an open gas volume must move toward external pressure");
}

void testHeatChangesGasState() {
    vf::GasVolumeState gas{};
    gas.volumeM3 = 1.0;
    gas.dryGasMassKg = 1.0;
    gas.temperatureK = 290.0;
    const double before = vf::gasPressurePa(gas);
    vf::addHeatToGas(gas, 10050.0, 1005.0);
...
int main() {
    testPressureRespondsToVolumeAndTemperature();
    testCoolingCreatesFogAndWarmingEvaporatesIt();
    testLeakMovesPressureTowardAmbient();
    testHeatChangesGasState();
