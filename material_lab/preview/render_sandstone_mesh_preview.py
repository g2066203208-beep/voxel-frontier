...
def main():
    ...
    for d in sorted(p for p in ROOT.iterdir() if p.is_dir()):
        ...
        h05,h50,h95=np.percentile(height,[5,50,95]);span=max(h95-h05,1e-5);S=1000;nlat=cfg["nlat"];nlon=cfg["nlon"]
        verts=[];uvs=[]
        ...
        tris=np.asarray(tris,np.int32);V=verts[tris];UV=uvs[tris];Rcam=2.35;zbuf=np.full((S,S),-1e9,np.float32);ou=np.zeros((S,S),np.float32);ov=np.zeros((S,S),np.float32);on=np.zeros((S,S,3),np.float32)
