...
import {
  bakeVfPainterlyDirt,
  bakeVfPainterlyBark,
  bakeVfPainterlyLeaves,
  bakeVfPainterlySnow,
  bakeVfPainterlyWater,
  type VfPainterlyDirtParams,
  type VfPainterlyBarkParams,
  type VfPainterlyLeavesParams,
  type VfPainterlySnowParams,
  type VfPainterlyWaterParams,
} from "./vf-recipes/vf-painterly-masters.js";
...
for (const req of requests) {
  if (!req.name || names.has(req.name)) throw new Error(`Invalid or duplicate material name: ${String(req.name)}`);
  names.add(req.name);
}
const manifests = requests.map(bakeOne);
console.log(JSON.stringify({ ok: true, styleId: PRODUCTION_STYLE_ID, materialCount: manifests.length, materials: manifests.map((m) => m.material) }, null, 2));
