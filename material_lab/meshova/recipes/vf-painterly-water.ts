...
export function bakeVfPainterlyWater(size:number,p:VfPainterlyWaterParams={}):VfPainterlyWaterBakeResult{
  ...
  for(let y=0;y<size;y++){
    ...
    for(let x=0;x<size;x++){
      ...
      const a=wavePhase(u,v,1.05*scale,.34*scale,p1,warp);
      const b=wavePhase(u,v,-.66*scale,1.58*scale,p2,-warp*.6);
      const c=wavePhase(u,v,2.55*scale,-1.12*scale,p3,warp*.35);
      const d=wavePhase(u,v,5.8*scale,3.25*scale,p4,-warp*.20);
      ...
      const combined=macro*cfg.macroAmp+midWave*cfg.midAmp+ripple*cfg.rippleAmp;
      const h=C(.50+relief*combined*.105);
      height.data[i]=h;
      crestSignal[i]=C(.5+.5*(macro*.68+midWave*.32));
  ...
  for(let y=0;y<size;y++){
    ...
    for(let x=0;x<size;x++){
      ...
      const foamCore=S((crest-cfg.crestThreshold)/Math.max(.06,1-cfg.crestThreshold));
      const foam=C(foamAmount*foamCore*(.35+.65*slope)*broken);
