...
function build(seed:number,density:number){
  const out:Leaf[]=[];const gx=Math.max(5,Math.round(6*density)),gy=Math.max(5,Math.round(6*density));let id=0;
  for(let y=0;y<gy;y++)for(let x=0;x<gx;x++)out.push({
    cx:(x+.5+(hash(seed,x,y,1)-.5)*.72)/gx,
    cy:(y+.5+(hash(seed,x,y,2)-.5)*.72)/gy,
    rx:(.48+hash(seed,x,y,3)*.42)/gx,
    ry:(.68+hash(seed,x,y,4)*.50)/gy,
    angle:(hash(seed,x,y,5)-.5)*2.7,
...
export function bakeVfPainterlyLeavesBundle(size:number,p:VfPainterlyLeavesParams={}){
  const seed=Math.floor(p.seed??391477),relief=Math.max(.5,Math.min(1.6,p.relief??1.06)),normalStrength=Math.max(2,Math.min(18,p.normalStrength??8.5)),density=Math.max(.72,Math.min(1.35,p.density??1)),leaves=build(seed,density);
  ...
  for(let py=0;py<size;py++){
    ...
    for(let px=0;px<size;px++){
      ...
      for(const leaf of leaves){
        ...
        const taper=Math.max(.14,1-Math.abs(y)*.76),q=Math.sqrt((x/taper)**2+y*y);
        if(q>1)continue;
        const inside=S((1-q)/.14),dome=Math.max(0,1-q*q),vein=G(x/.070)*S((1-Math.abs(y))/.18);
        const serration=Math.sin(TAU*(7*Math.abs(y)+leaf.id*.17))*.0045*inside;
        const h=leaf.base+relief*(dome*.090+inside*.018+vein*.010+serration);
        if(h>bestH){second=bestH;bestH=h;best=leaf;bestInside=inside;bestVein=vein}
        else if(h>second)second=h;
      }

      const overlap=C((bestH-second)/.10),cavity=C(1-overlap);
      let col:RGB;
      if(best){
        const variation=best.hue;
        col=M(dark,mid,.38+variation*.38);
        col=M(col,light,bestInside*(.26+variation*.20));
        col=M(col,warm,C(variation-.64)*.15);
        col=M(col,cool,C(.36-variation)*.15);
        col=M(col,[.520,.570,.235],bestVein*.08);
        col=M(col,deep,cavity*.34);
      }else col=deep;
      const wash=periodicField(u,v,seed+701,3);
      col=M(col,wash>0?light:cool,Math.abs(wash)*.032);

      baseColor.data[j]=C(col[0]);baseColor.data[j+1]=C(col[1]);baseColor.data[j+2]=C(col[2]);
      height.data[i]=C(bestH);
      roughness.data[i]=C(best?.73+cavity*.08:.92);
      ao.data[i]=best?C(1-cavity*.36):1;
      opacity.data[i]=best?C(S((bestInside-.12)/.55)):0;
    }
  }
