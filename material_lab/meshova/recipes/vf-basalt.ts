...
function pnoise(u: number, v: number, frequency: number, seed: number): number {
  ...
  const ax = wrap(x0), bx = wrap(x0 + 1), ay = wrap(y0), by = wrap(y0 + 1);
  const n00 = hash01(ax, ay, seed), n10 = hash01(bx, ay, seed);
  const n01 = hash01(ax, by, seed), n11 = hash01(bx, by, seed);
...
function signedSharpen(x: number, exponent: number): number {
  ...
  return (centered < 0 ? -1 : 1) * Math.pow(Math.abs(centered), exponent) * 0.5;
}

function sparsePores(u: number, v: number, seed: number, density: number): number {
  const cells = 24;
  const px = u * cells, py = v * cells;
  const baseX = Math.floor(px), baseY = Math.floor(py);
  let mask = 0;
  for (let oy = -1; oy <= 1; oy++) {
    for (let ox = -1; ox <= 1; ox++) {
      ...
      const dx = px - cx - jx, dy = py - cy - jy;
      const rx = dx * c - dy * s, ry = dx * s + dy * c;
      const d = Math.hypot(rx / radius, ry / (radius * aspect));
      mask = Math.max(mask, 1 - smooth(clamp01((d - 0.40) / 0.60)));
    }
  }
  return mask;
}
...
function makeSegment(
  cx: number, cy: number, angle: number, halfLength: number, width: number,
  strength: number, bend: number, phase: number,
): CrackSegment {
  return {
    cx: wrap01(cx), cy: wrap01(cy), dx: Math.cos(angle), dy: Math.sin(angle),
    halfLength, width, strength, bend, phase,
  };
}
...
function buildFractures(seed: number): CrackSegment[] {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const angle = familyAngle + (hash01(i, 1, seed, 100) - 0.5) * 0.66;
    const segment = makeSegment(
      hash01(i, 2, seed, 101), hash01(i, 3, seed, 102), angle,
  ...
  for (let i = 0; i < 12; i++) {
    ...
    segments.push(makeSegment(
      hash01(i, 21, seed, 122), hash01(i, 22, seed, 123), angle,
...
function segmentFields(u: number, v: number, segment: CrackSegment, edgeBreakup: number): [number, number] {
  let rx = u - segment.cx, ry = v - segment.cy;
  rx -= Math.round(rx);
  ry -= Math.round(ry);
  ...
  const inner = 1 - smooth(clamp01((cross - segment.width * 0.28) / (segment.width * 0.65)));
  return [
    clamp01(core * alongMask * segment.strength),
    clamp01((outer - inner) * alongMask * segment.strength),
  ];
}
...
function makeSampler(params: VfBasaltParams) {
  ...
  const normalStrength = Math.max(0.2, Math.min(14, params.normalStrength ?? 7.6));
  const macroRelief = Math.max(0.5, Math.min(1.8, params.macroRelief ?? 1.16));
  const mesoRelief = Math.max(0.5, Math.min(2.0, params.mesoRelief ?? 1.32));
  ...
  const sample = (u: number, v: number) => {
    ...
    const mesoSharp = signedSharpen(pfbm(du, dv, 10, 3, seed + 251), 0.72);
    const ridgeField = pfbm(du, dv, 6, 4, seed + 263);
    const ridge = Math.pow(clamp01(1 - Math.abs((ridgeField - 0.5) * 2)), 2.35) - 0.34;
    const facet = signedSharpen(pfbm(du, dv, 17, 2, seed + 271), 0.82);
    ...
    let fracture = 0, fractureLip = 0;
    for (const segment of fractures) {
      const [core, lip] = segmentFields(du, dv, segment, edgeBreakup);
      fracture = Math.max(fracture, core);
      fractureLip = Math.max(fractureLip, lip);
    }
    ...
    const pitA = smooth(clamp01((pnoise(du, dv, 73, seed + 809) - 0.835) / 0.10));
    const pitB = smooth(clamp01((pnoise(du, dv, 137, seed + 811) - 0.885) / 0.075));
    const pinholes = clamp01(pitA * 0.32 + pitB * 0.24);
    ...
    const height = clamp01(
      ...
      + (meso - 0.5) * 0.20 * mesoRelief
      + mesoSharp * 0.100 * mesoRelief
      + ridge * 0.070 * mesoRelief
      + facet * 0.042 * mesoRelief
      + (micro - 0.5) * 0.080
      + (grain - 0.5) * 0.030
    ...
    color = color.map((c) => clamp01(c * cavityDark * grainTone)) as RGB;
    ...
    const roughness = clamp01(
      ...
      + (microGrain - 0.5) * 0.10
      + (grit - 0.5) * 0.06
      + fracture * 0.18 + fractureLip * 0.07
      + pores * 0.22 + pinholes * 0.10
      + darkMineral * 0.05 - brightMineral * 0.11
      - height * 0.030,
    );
    const ao = clamp01(1 - fracture * 0.42 - pores * 0.62 - pinholes * 0.14 - Math.max(0, -mesoSharp) * 0.07);
